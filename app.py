...
if dfs:
    ...
    if not df_cars_raw.empty:
        ...
        for row in raw_values:
            ...
            checkup_date = pd.to_datetime(get('BD'), errors='coerce') # New Checkup Date
            ins_end = pd.to_datetime(get('BJ'), errors='coerce') # New Insurance Date source
            has_ins = 'Yes' in str(get('BN') or '') or 'نعم' in str(get('BN') or '')
            
            contract_end = pd.to_datetime(get('BC'), errors='coerce') # UPDATED to BC
            
            # Payment Logic
            base_amt = clean_money(get('CJ')) # 30-day rate
            pay_freq = clean_money(get('CK')) # e.g. 45 days
            pay_deduct_pct = clean_money(get('CL')) # Percentage to deduct
            pay_start = pd.to_datetime(get('CM'), errors='coerce') # Shifted start date to CM based on your update
            
            # Calculate Actual Payment Amount
            # Formula: (Base / 30) * Frequency
            if pay_freq > 0:
                calc_amt = (base_amt / 30) * pay_freq
            else:
                calc_amt = base_amt
            
            # Apply Deduction
            final_amt = calc_amt * (1 - (pay_deduct_pct / 100))
            
            cars_clean.append({
                'Code': code, 'Full_Name': c_name, 'Type': c_type, 'Model': c_model, 'Year': c_year, 'Plate': plate,
                'Active': is_active, 'KM': km_start, 
    ...
    tabs = st.tabs(["📅 Calendar", "🚗 Fleet", "💰 Financials", "⚠️ Risk Management", "🤝 Owner Payments", "📋 Full Database", "🧠 AI"])
    ...
    with tabs[1]:
        ...
        with c1:
            st.subheader(f"Active Fleet ({len(active_cars)}) - Sorted by Contract End")
        ...
        with c2:
            st.subheader("Inactive Fleet (Sorted by Latest)")
    ...
    with tabs[3]:
        ...
        for _, car in df_cars.iterrows():
            ...
            if pd.notnull(lic_date):
                # If Checkup is same as License, rename to "License + Exam"
            ...
            # Insurance Check (Only if 'Has_Ins' is True)
        ...
        else: st.success("✅ No upcoming expiries.")
